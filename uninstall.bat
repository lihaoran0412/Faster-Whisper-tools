...
echo   Cleaning HuggingFace hub shared blobs...
if exist "%USERPROFILE%\.cache\huggingface\hub\.locks" (
    rmdir /s /q "%USERPROFILE%\.cache\huggingface\hub\.locks" 2>nul
)
if exist "%USERPROFILE%\.cache\huggingface\hub\blobs" (
    rmdir /s /q "%USERPROFILE%\.cache\huggingface\hub\blobs" 2>nul
    echo   Removed: shared blob cache
)
